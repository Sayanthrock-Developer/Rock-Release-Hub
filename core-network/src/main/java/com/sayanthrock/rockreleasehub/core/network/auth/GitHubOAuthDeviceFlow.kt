...
class GitHubOAuthDeviceFlowGateway @Inject constructor() : OAuthDeviceFlowGateway {
    ...
    override suspend fun awaitAccessToken(
        ...
        while (SystemClock.elapsedRealtime() < deadline) {
            ...
            val response = try {
                attemptTokenFetch(clientId, authorization.deviceCode)
            ...
            pollingInterval += processPollingError(response)
        }

        handleTimeout(lastNetworkFailure)
    }

    private fun handleTimeout(lastNetworkFailure: IOException?): Nothing {
    ...
    private fun processPollingError(response: JSONObject): Long {
        when (val error = response.optString("error")) {
            "authorization_pending" -> return 0L
            "slow_down" -> return SLOW_DOWN_INCREMENT_SECONDS
    ...
    private fun attemptTokenFetch(clientId: String, deviceCode: String): JSONObject {
        return postForm(
